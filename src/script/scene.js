...
const mousePosition     = new THREE.Vector2(1, 1,1);
const initMousePosition = new THREE.Vector2(1, 1,1);
const multiplier =18;
let cubePosition      = new THREE.Vector3(1, 1,1);
let cubeRotation      = new THREE.Vector3(1, 1,1);
let cubeScale         = new THREE.Vector3(1, 1,1);
let cameraPosition    = new THREE.Vector3(1, 1,1);
let cameraScreen   = 0;
...
// const getMaxFrustumSize = (scaleValue)=>{
//     const value = {x:scaleValue.x,y:scaleValue.y,z:scaleValue.z};
//     if(Math.abs(value.x)>frustumSize){
//         frustumSize = Math.abs(value.x)*.74;
//         console.log("  !!! ",frustumSize)
//     }
//     else if(Math.abs(value.y)>frustumSize){
//         frustumSize = Math.abs(value.y);
//         console.log("  222222222 ",frustumSize);
//     }
//     else if(Math.abs(value.z)>frustumSize){
//         frustumSize = Math.abs(value.z)*.74;
//         console.log("  333333333333333 ",frustumSize);
//     }
//     const camera = views[1].camera;
//     const aspectRatio = window.innerWidth / window.innerHeight;
//     // frustumSize =9;
//      if(frustumSize>intialFrustum){
...
const onMouseDown = (event)=> {
    ...
    //      (views[1].camera.near+views[1].camera.far)/(views[1].camera.near-views[1].camera.far),
    //   );
    // mouseWorld.unproject(views[1].camera);
    ...
    if (mousePosition.x > 0 && mousePosition.y < 0) 
      ...
      vector.set( mousePosition.x, mousePosition.y, ( views[2].camera.near + views[2].camera.far ) / ( views[2].camera.near - views[2].camera.far ) );
      vector.unproject( views[2].camera );
    //  const pos = new THREE.Vector3(mousePosition.x,mousePosition.y,.5);
    //  pos.unproject(views[1].camera);
